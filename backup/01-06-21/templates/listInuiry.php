...
if($_REQUEST['inqueryStartDate']!=""){
  global $wpdb;  
    ...
    $totalGrand = $wpdb->num_rows;
      ...
      $inquirey1 = $wpdb->get_results(" Select * from lb_inquirey where datetime >=  '".$_REQUEST['inqueryStartDate']."' AND datetime <='".date('Y-m-d H:i:s', strtotime($_REQUEST['inqueryToDate'] . ' +1 day'))."' and inqueryMethod='phonecall'  ORDER BY id DESC ");
         $countPhoneCall = $wpdb->num_rows;

        
         $inquirey2 = $wpdb->get_results(" Select * from lb_inquirey where datetime >=  '".$_REQUEST['inqueryStartDate']."' AND datetime <='".date('Y-m-d H:i:s', strtotime($_REQUEST['inqueryToDate'] . ' +1 day'))."'  and inqueryMethod='text'  ORDER BY id DESC ");
         //$countTextCall = $inquirey2->num_rows;
          $countTextCall = $wpdb->num_rows;

         $inquirey3 = $wpdb->get_results(" Select * from lb_inquirey where datetime >=  '".$_REQUEST['inqueryStartDate']."' AND datetime <='".date('Y-m-d H:i:s', strtotime($_REQUEST['inqueryToDate'] . ' +1 day'))."'  and inqueryMethod='email'  ORDER BY id DESC ");
        ...
        <?php if($_REQUEST['inqueryStartDate']!=""){
          ...
          ?>
            <tr>
                ...
                <td><?php echo $getVal->inqueryMethod;?></td>
