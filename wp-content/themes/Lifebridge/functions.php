...
<div class="row">
<div class="col-sm-12">
<input placeholder="Username" name="isigma_user_loginu" id="isigma_user_loginu" class="required" required="" type="text"/>
</div>
<div class="col-sm-12">
<input placeholder="Password" required="" name="isigma_user_passlo" id="isigma_user_passlo" class="required" type="password"/>
</div>
...
<input id="isigma_login_submit" class="submtBtn" type="submit" value="Login"/>
</form>
</div>
...
//echo "<pre>"; print_r($user); die;
 if($request == ''){
wp_redirect(home_url().'/redirected-user/'); exit;
 } else{
...
$custom_table_example_db_version = '5.7.2'; // version changed from 1.0 to 1.1
...
class Custom_Table_Example_List_Table extends WP_List_Table
{
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'patientId' => __('Patient id', 'custom_table_example'),
            'age' => __('Age', 'custom_table_example'),
            'gender' => __('Gender', 'custom_table_example'),
            'employmentstatus' => __('Employment Status', 'custom_table_example'),
            'specialty' => __('Specialty', 'custom_table_example'),
            'counselor' => __('Counselor', 'custom_table_example'),
            'appointmentType' => __('Appointment Type', 'custom_table_example'),
            'dateOfSession' => __('Datev Of Session', 'custom_table_example'),
            'visitNumber' => __('Visit Number', 'custom_table_example'),
            'billable' => __('Billable', 'custom_table_example'),
        );
        return $columns;
    }
    ...
    function get_sortable_columns()
    {
        $sortable_columns = array(
            'patientId' => array('patientId', true),
            'age' => array('age', false),
            'gender' => array('gender', false),
            'employmentstatus' => array('employmentstatus', false),
            'specialty' => array('specialty', false),
            'counselor' => array('counselor', false),
            'appointmentType' => array('appointmentType', false),
            'dateOfSession' => array('dateOfSession', false),
            'visitNumber' => array('visitNumber', false),
            'billable' => array('billable', false),
        );
    ...
    {
        global $wpdb;
        $table_name = $wpdb->prefix . 'appointment'; // do not forget about tables prefix
...
{
    add_menu_page(__('Appointment', 'custom_table_example'), __('Appointment', 'custom_table_example'), 'activate_plugins', 'appointment', 'custom_table_example_appointment_page_handler');
    add_submenu_page('appointment', __('Appointment', 'custom_table_example'), __('Appointment', 'custom_table_example'), 'activate_plugins', 'appointment', 'custom_table_example_appointment_page_handler');
...
/**
    ...
    */
function custom_table_example_appointment_page_handler()
{
    global $wpdb;
...
/**
    * custom_inquery_page_handler_Table_Example_List_Table class that will display our custom table
    * records in nice table
    */
class Custom_Table_inquirey_List_Table extends WP_List_Table
{
    /**
        * [REQUIRED] You must declare constructor and give some basic params
        */
    ...
    function get_columns()
    {
        $columns = array(
            'cb' => '<input type="checkbox" />', //Render a checkbox instead of text
            'inqueryDate' => __('Date', 'custom_table_inquirty'),
            'inquerytime' => __('Time', 'custom_table_inquirty'),
            'inqueryMethod' => __('Method', 'custom_table_inquirty'),
        );
        return $columns;
    }
    ...
    {
        $sortable_columns = array(
            'inqueryDate' => array('inqueryDate', true),
            'inquerytime' => array('inquerytime', false),
            'inqueryMethod' => array('inqueryMethod', false),
        );
        return $sortable_columns;
    }
    ...
    {
        global $wpdb;
        $table_name = $wpdb->prefix . 'inquirey'; // do not forget about tables prefix
...
/**
    * admin_menu hook implementation, will add pages to list persons and to add new one
    */
function custom_table_inquery_admin_menu()
{
    add_menu_page(__('Inquery', 'custom_table_example'), __('Inquery', 'custom_table_example'), 'activate_plugins', 'inquirey', 'custom_table_example_inquery_page_handler');
    add_submenu_page('inquirey', __('Inquery', 'custom_table_example'), __('Inquery', 'custom_table_example'), 'activate_plugins', 'inquirey', 'custom_table_example_inquery_page_handler');
    // add new will be described in next part

    //add_submenu_page('persons', __('Add new', 'custom_table_example'), __('Add new', 'custom_table_example'), 'activate_plugins', 'persons_form', 'custom_table_example_persons_form_page_handler');
}

add_action('admin_menu', 'custom_table_inquery_admin_menu');
...
/**
    ...
    * Look into /wp-admin/includes/class-wp-*-list-table.php for examples
    */
function custom_table_example_inquery_page_handler()
{
    global $wpdb;

    $table = new Custom_Table_inquirey_List_Table();
    $table->prepare_items();
...
function custom_table_inquiery_languages()
{
    load_plugin_textdomain('custom_table_inquirty', false, dirname(plugin_basename(__FILE__)));
}

add_action('init', 'custom_table_inquiery_languages');